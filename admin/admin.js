...
const EMPTY = {
  ...
  landingServices: [],
  bookingConfig: null,
  landingHero: { image: "", alt: { pl: "", en: "", ru: "" } },
  contacts: {},
  socials: {}
};
...
function normalizeState(raw) {
  ...
  if (!o.landingHero || typeof o.landingHero !== "object") {
    o.landingHero = { image: "", alt: { pl: "", en: "", ru: "" } };
  } else {
    if (typeof o.landingHero.image !== "string") o.landingHero.image = "";
    if (!o.landingHero.alt || typeof o.landingHero.alt !== "object") {
      o.landingHero.alt = { pl: "", en: "", ru: "" };
    }
    ["pl", "en", "ru"].forEach((k) => {
      if (o.landingHero.alt[k] === undefined) o.landingHero.alt[k] = "";
    });
  }
...
function ensureLandingHero() {
  if (!state.landingHero || typeof state.landingHero !== "object") {
    state.landingHero = { image: "", alt: { pl: "", en: "", ru: "" } };
  }
  if (typeof state.landingHero.image !== "string") state.landingHero.image = "";
  if (!state.landingHero.alt || typeof state.landingHero.alt !== "object") {
    state.landingHero.alt = { pl: "", en: "", ru: "" };
  }
  ["pl", "en", "ru"].forEach((k) => {
    if (state.landingHero.alt[k] === undefined) state.landingHero.alt[k] = "";
  });
}
...
function updateDirtyBanner() {
  ...
  el.classList.toggle("salon-dirty", dirty);
}
...
function showSave(msg, kind) {
  ...
  if (!el) return;
  el.textContent = msg || "";
  el.className = "salon-status";
  if (kind === "ok") el.classList.add("salon-status--ok");
  if (kind === "err") el.classList.add("salon-status--err");
}
...
function bindCardExpandDelegation() {
  ...
  app.addEventListener("click", (e) => {
    const btn = e.target.closest(".salon-card-edit-toggle");
    if (!btn || !app.contains(btn)) return;
    const card = btn.closest(".salon-card");
    if (!card) return;
...
function expandCardForEdit(card) {
  ...
  const btn = card.querySelector(".salon-card-edit-toggle");
  if (btn) {
...
function focusFirstEditableInCard(card) {
  if (!card) return;
  const panel = card.querySelector(".salon-card-edit-panel");
...
function scrollToNewCard(card) {
  ...
  card.classList.add("salon-card--flash");
  window.setTimeout(() => card.classList.remove("salon-card--flash"), 1600);
}

function afterNewCardInList(container, listKey) {
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      const card = container.querySelector(`.salon-item.salon-card[data-list="${listKey}"]:last-of-type`);
      if (!card) return;
...
const LOCALE_FIELD_LABEL = {
  ...
  heroAlt: {
    pl: "Opis zdjęcia hero — PL (dostępność / SEO)",
    ru: "Opis zdjęcia hero — RU",
    en: "Opis zdjęcia hero — EN"
  }
};
...
function fieldLocaleBlock(item, pathStr, mode) {
  ...
  const copyRow =
    ...
    mode === "bookingSvcDesc" ||
    mode === "heroAlt"
      ? `<div class="salon-field salon-field--tight">
           <button type="button" class="salon-btn salon-btn--ghost salon-btn--small" data-copy-pl-to-empty="${esc(
             pathStr
  ...
  const rows = ["pl", "ru", "en"].map((lang) => {
    ...
    if (multiline) {
      return `<div class="salon-field"><label>${esc(lab)}</label><textarea data-locale-path="${esc(
        pathStr
      )}" data-lang="${lang}">${v}</textarea></div>`;
    }
    return `<div class="salon-field"><label>${esc(lab)}</label><input type="text" data-locale-path="${esc(
      pathStr
    )}" data-lang="${lang}" value="${v}" /></div>`;
  });
  return `${copyRow}<div class="salon-locale-grid">${rows.join("")}</div>`;
}
...
function fieldLocaleAltSeoBlock(item) {
  ...
  return `<div class="salon-locale-grid">${rows.join("")}</div>`;
}
...
function emptyStateBlock(listKey) {
  ...
  if (!lines) return "";
  return `<div class="salon-empty" role="status">
    <p>${esc(lines[0])}</p>
    <p class="salon-empty__sub">${esc(lines[1])}</p>
  </div>`;
}
...
function updateGalleryCardPreviews(wrap, item) {
  ...
  const heroBox = wrap.querySelector(".salon-card-hero__media");
  if (heroBox) {
    if (!src) heroBox.innerHTML = `<span class="salon-preview-placeholder">Brak zdjęcia</span>`;
    else if (type === "video") {
      heroBox.innerHTML = `<video class="salon-preview" src="${esc(src)}" muted loop playsinline controls></video>`;
    } else {
      heroBox.innerHTML = `<img alt="" src="${esc(src)}" />`;
    }
  }
  const box = wrap.querySelector(".salon-preview--main");
  if (box) {
    if (!src) box.innerHTML = "";
    else if (type === "video") {
      box.innerHTML = `<video class="salon-preview" src="${esc(src)}" muted loop playsinline controls></video>`;
    } else {
      box.innerHTML = `<img alt="" src="${esc(src)}" />`;
    }
  }
  const mini = wrap.querySelector(".salon-card-preview-media");
  if (mini) {
    if (!src) mini.innerHTML = "";
    else if (type === "video") {
      mini.innerHTML = `<video class="salon-preview" src="${esc(src)}" muted loop playsinline></video>`;
    } else {
  ...
  if (badge) {
    ...
    badge.classList.toggle("salon-badge--off", item.visible === false);
  }
}
...
function renderGalleryEditor(container, listKey, options) {
  ...
  const head = `
    <button type="button" class="salon-btn salon-btn--primary salon-btn--block-sm" data-add="${esc(listKey)}">${esc(addLabel)}</button>
    <p class="salon-hint">${esc(hint)}</p>
  ...
  const blocks = list
    .map((item, idx) => {
      ...
      return `
      <div class="salon-item salon-card" data-list="${esc(listKey)}" data-index="${idx}" data-preview-lang="pl">
        <div class="salon-card-hero">
          <div class="salon-card-hero__media salon-preview-box">${src ? "" : `<span class="salon-preview-placeholder">Brak zdjęcia</span>`}</div>
          <div class="salon-card-hero__meta">
            <strong class="salon-card-hero__title" data-card-head-title>${esc(cardTitleLine(item, "pl"))}</strong>
            <span class="salon-badge" data-card-visibility-badge>${item.visible !== false ? "Widoczna na stronie" : "Ukryta na stronie"}</span>
            <div class="salon-card-hero__actions">
              <button type="button" class="salon-btn" data-move="${esc(listKey)}" data-dir="-1" data-index="${idx}">Wyżej</button>
              <button type="button" class="salon-btn" data-move="${esc(listKey)}" data-dir="1" data-index="${idx}">Niżej</button>
              <button type="button" class="salon-btn salon-btn--danger" data-del="${esc(listKey)}" data-index="${idx}">Usuń kartę</button>
            </div>
          </div>
        </div>

        <button type="button" class="salon-btn salon-btn--ghost salon-card-edit-toggle" aria-expanded="false">Edytuj</button>
        <div class="salon-card-edit-panel">

        <h4 class="salon-section-title">1. Co klient zobaczy?</h4>
        <p class="salon-field-hint salon-field-hint--above">Ta nazwa i opis pokażą się klientowi na stronie.</p>
        ${fieldLocaleBlock(item, "title", "name")}
        ${fieldLocaleBlock(item, "description", "desc")}

        <h4 class="salon-section-title">2. Zdjęcie albo video</h4>
        <p class="salon-field-hint salon-field-hint--above">Wgraj plik z telefonu lub komputera. Po wgraniu pojawi się tutaj automatycznie.</p>
        ${typeSelect}
        <div class="salon-field">
          <label>Wgraj zdjęcie albo video</label>
          <input type="file" class="salon-file-input" data-upload="${esc(listKey)}" data-index="${idx}" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime,.mp4,.webm,.mov" />
        </div>
        <p class="salon-field-hint">Podgląd zdjęcia:</p>
        <div class="salon-preview salon-preview--main"></div>
        ${
          listKey === "worksGallery"
            ? `<div class="salon-field salon-field--tight">
          <button type="button" class="salon-btn salon-btn--ghost salon-btn--small" data-use-work-as-hero="${idx}" ${
            type === "video" ? "disabled" : ""
          }>Użyj tego zdjęcia jako hero strony</button>
          <p class="salon-field-hint">${
            type === "video"
              ? "Hero obsługuje tylko zdjęcia (nie wideo)."
              : "Ustawia to samo zdjęcie i opisy ALT w sekcji Hero (zakładka Kontakt)."
          }</p>
        </div>`
            : ""
        }

        <h4 class="salon-section-title">3. Czy pokazywać na stronie?</h4>
        <div class="salon-field salon-field--inline">
          <label>
        ...
        <div class="salon-field">
          ...
          <p class="salon-field-hint">Niższa liczba = pokaże się wcześniej.</p>
        </div>

        <details class="salon-details">
          <summary>Dodatkowe ustawienia — nie musisz tego ruszać</summary>
          <p class="salon-field-hint">Dla wyszukiwarek. Możesz zostawić puste.</p>
          ${fieldLocaleAltSeoBlock(item)}
        </details>

        <details class="salon-details salon-details--tech">
          <summary>Ustawienia techniczne — tylko dla programisty</summary>
          <p class="salon-field-hint">Wybrane zdjęcie albo film — zwykle uzupełnia się samo po wgraniu pliku.</p>
          <div class="salon-field">
        ...
        <div class="salon-card-preview-block">
          <h4 class="salon-section-title">Podgląd karty</h4>
          <div class="salon-preview-lang" role="group" aria-label="Podgląd języka">
            <span>Język:</span>
            <button type="button" class="salon-lang-pill active" data-preview-lang-btn="pl">PL</button>
            <button type="button" class="salon-lang-pill" data-preview-lang-btn="ru">RU</button>
            <button type="button" class="salon-lang-pill" data-preview-lang-btn="en">EN</button>
          </div>
          <p class="salon-field-hint">Tak karta wygląda dla klienta.</p>
          <div class="salon-card-preview-card">
            <div class="salon-card-preview-media salon-preview-box"></div>
  ...
  list.forEach((item, idx) => {
    const wrap = container.querySelector(`.salon-item[data-index="${idx}"][data-list="${listKey}"]`);
    if (!wrap) return;
    ...
    wrap.querySelector("[data-field=mediaType]")?.addEventListener("change", (e) => {
      ...
      const heroBtn = wrap.querySelector(`[data-use-work-as-hero="${idx}"]`);
      if (heroBtn) {
        heroBtn.disabled = e.target.value === "video";
      }
      updateGalleryCardPreviews(wrap, item);
      updateDirtyBanner();
    });
    ...
    wrap.querySelector(`[data-use-work-as-hero="${idx}"]`)?.addEventListener("click", () => {
      if (listKey !== "worksGallery") return;
      const it = state.worksGallery[idx];
      if (!it) return;
      const mType = it.media?.type || "image";
      if (mType === "video") return;
      const raw = (it.media?.src || "").trim();
      if (!raw) {
        showSave("Najpierw dodaj zdjęcie do tej karty.", "err");
        return;
      }
      ensureLandingHero();
      state.landingHero.image = raw.startsWith("/") ? raw : `/${raw.replace(/^\.?\//, "")}`;
      const altRef = ensureLocaleRef(state.landingHero, "alt");
      const mAlt = it.media?.alt;
      if (mAlt && typeof mAlt === "object") {
        ["pl", "ru", "en"].forEach((L) => {
          const v = String(mAlt[L] || "").trim();
          if (v) altRef[L] = v;
        });
      }
      showSave("Ustawiono zdjęcie hero — zapisz zmiany.", "ok");
      updateDirtyBanner();
      const cp = document.getElementById("panel-contacts");
      if (cp && cp.classList.contains("active")) renderContactsEditor(cp);
    });
    ...
    wrap.querySelector(`[data-upload="${listKey}"][data-index="${idx}"]`)?.addEventListener("change", async (e) => {
      ...
      try {
        ...
        const heroBtnUp = wrap.querySelector(`[data-use-work-as-hero="${idx}"]`);
        if (heroBtnUp && listKey === "worksGallery") heroBtnUp.disabled = item.media.type === "video";
        updateGalleryCardPreviews(wrap, item);
...
async function uploadFile(file) {
  ...
  fd.append("file", file);
  const res = await fetch("/api/admin/upload", fetchCred({
    method: "POST",
    headers: authHeaders(),
    body: fd
  }));
...
function renderBarbersEditor(container) {
  ...
  const head = `
    <button type="button" class="salon-btn salon-btn--primary salon-btn--block-sm" data-add-barber>+ Dodaj barbera</button>
    <p class="salon-hint">Zdjęcie, imię i krótki opis — tak wygląda karta na stronie.</p>
  ...
  const blocks = list
    .map((item, idx) => {
      ...
      const rowChecks = BARBER_SERVICE_CHECKBOXES.map(
        (s) => `
        <label class="salon-check">
          <input type="checkbox" data-barber-svc="${esc(s.id)}" ${checked(s.id)} />
          <span>${esc(s.label)}</span>
        </label>`
      ).join("");
      return `
      <div class="salon-item salon-card" data-list="barbers" data-index="${idx}" data-preview-lang="pl">
        <div class="salon-card-hero">
          <div class="salon-card-hero__media salon-preview-box">${src ? `<img alt="" src="${esc(src)}" />` : `<span class="salon-preview-placeholder">Brak zdjęcia</span>`}</div>
          <div class="salon-card-hero__meta">
            <strong class="salon-card-hero__title" data-card-head-title>${esc(cardTitleLine(item, "pl"))}</strong>
            <span class="salon-badge" data-card-visibility-badge>${item.visible !== false ? "Widoczna na stronie" : "Ukryta na stronie"}</span>
            <div class="salon-card-hero__actions">
              <button type="button" class="salon-btn" data-move="barbers" data-dir="-1" data-index="${idx}">Wyżej</button>
              <button type="button" class="salon-btn" data-move="barbers" data-dir="1" data-index="${idx}">Niżej</button>
              <button type="button" class="salon-btn salon-btn--danger" data-del="barbers" data-index="${idx}">Usuń kartę</button>
            </div>
          </div>
        </div>
        ...
        <h4 class="salon-section-title">1. Co klient zobaczy?</h4>
        <p class="salon-field-hint salon-field-hint--above">Ta nazwa i opis pokażą się na stronie.</p>
        ${fieldLocaleBlock(item, "title", "barberName")}
        ${fieldLocaleBlock(item, "description", "desc")}
        ${fieldLocaleBlock(item, "bookCta", "bookCta")}

        <h4 class="salon-section-title">2. Zdjęcie barbera</h4>
        <p class="salon-field-hint salon-field-hint--above">Wgraj zdjęcie z telefonu lub komputera — pojawi się tu automatycznie.</p>
        <div class="salon-field">
          <label>Wgraj zdjęcie</label>
          <input type="file" class="salon-file-input" data-upload-barber="${idx}" accept="image/jpeg,image/png,image/webp,image/gif" />
        </div>
        <p class="salon-field-hint">Podgląd zdjęcia:</p>
        <div class="salon-preview salon-preview--main">${src ? `<img alt="" src="${esc(src)}" />` : ""}</div>

        <h4 class="salon-section-title">3. Małe tagi pod opisem</h4>
        <div class="salon-field">
          <label>Jedna linia = jeden tag</label>
          <textarea data-field="tags">${esc(tagsStr)}</textarea>
        </div>

        <h4 class="salon-section-title">4. Czy pokazywać na stronie?</h4>
        <div class="salon-field salon-field--inline">
        ...
        <h4 class="salon-section-title">5. Jakie usługi wykonuje?</h4>
        <p class="salon-field-hint salon-field-hint--above">Dla Ciebie — pełna lista jest w „Rezerwacja online”.</p>
        <div class="salon-check-grid">${rowChecks}</div>
        ...
        <details class="salon-details salon-details--tech">
          <summary>Ustawienia techniczne — tylko dla programisty</summary>
          <p class="salon-field-hint">Tu są rzadko potrzebne rzeczy wewnętrzne — zwykle nie ruszaj.</p>
          <div class="salon-field">
        ...
        <div class="salon-card-preview-block">
          <h4 class="salon-section-title">Podgląd</h4>
          <div class="salon-preview-lang" role="group" aria-label="Podgląd języka">
            <span>Język:</span>
            <button type="button" class="salon-lang-pill active" data-preview-lang-btn="pl">PL</button>
            <button type="button" class="salon-lang-pill" data-preview-lang-btn="ru">RU</button>
          ...
          <p class="salon-field-hint">Tak karta wygląda dla klienta.</p>
          <div class="salon-card-preview-card salon-card-preview-card--barber">
            <div class="salon-card-preview-media salon-preview-box"></div>
            <strong data-card-preview-title></strong>
            <p data-card-preview-desc></p>
            <button type="button" class="salon-preview-fake-btn" data-card-preview-btn>Umów</button>
          </div>
        </div>
  ...
  list.forEach((item, idx) => {
    const wrap = container.querySelector(`.salon-item[data-index="${idx}"][data-list="barbers"]`);
    if (!wrap) return;
...
function renderLandingServicesEditor(container) {
  ...
  const head = `
    <button type="button" class="salon-btn salon-btn--primary salon-btn--block-sm" data-add-landing>+ Dodaj usługę na stronę</button>
    <p class="salon-hint">To są karty na stronie głównej — nie myl z listą w rezerwacji. Pusta lista = zostaje tekst początkowy.</p>
  ...
  const blocks = list
    .map((item, idx) => {
      ...
      return `
      <div class="salon-item salon-card" data-list="${listKey}" data-index="${idx}" data-preview-lang="pl">
        <div class="salon-card-hero">
          <div class="salon-card-hero__media salon-preview-box"><span class="salon-preview-placeholder">Podgląd</span></div>
          <div class="salon-card-hero__meta">
            <strong class="salon-card-hero__title" data-card-head-title>${esc(cardTitleLine(item, "pl"))}</strong>
            <span class="salon-badge" data-card-visibility-badge>${item.visible !== false ? "Widoczna na stronie" : "Ukryta na stronie"}</span>
            <div class="salon-card-hero__actions">
              <button type="button" class="salon-btn" data-move="${listKey}" data-dir="-1" data-index="${idx}">Wyżej</button>
              <button type="button" class="salon-btn" data-move="${listKey}" data-dir="1" data-index="${idx}">Niżej</button>
              <button type="button" class="salon-btn salon-btn--danger" data-del="${listKey}" data-index="${idx}">Usuń kartę</button>
            </div>
          </div>
        </div>
        ...
        <h4 class="salon-section-title">1. Co klient zobaczy?</h4>
        <p class="salon-field-hint salon-field-hint--above">Nazwa, opis i cena na karcie.</p>
        ${fieldLocaleBlock(item, "title", "name")}
        ${fieldLocaleBlock(item, "description", "desc")}
        <div class="salon-field">
        ...
        <div class="salon-field">
          ...
          <p class="salon-field-hint salon-field-hint--tight">Wybierz usługę z listy — albo zostaw „brak”, jeśli przycisk ma tylko otworzyć rezerwację.</p>
          <select data-field="bookingServiceId">${opts}</select>
        </div>

        <h4 class="salon-section-title">2. Zdjęcie albo krótki film</h4>
        <p class="salon-field-hint salon-field-hint--above">Możesz użyć gotowego stylu karty albo wgrać własny plik.</p>
        <div class="salon-field">
        ...
        <div class="salon-field">
          ...
          <input type="file" class="salon-file-input" data-upload="${listKey}" data-index="${idx}" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime,.mp4,.webm,.mov" />
        </div>
        <p class="salon-field-hint">Podgląd:</p>
        <div class="salon-preview salon-preview--main">${src ? `<img alt="" src="${esc(src)}" />` : ""}</div>
        <div class="salon-field">
          <label>Małe tagi pod opisem</label>
          <textarea data-field="tags">${esc(tagsStr)}</textarea>
          <p class="salon-field-hint">Jedna linia = jeden tag.</p>
        </div>
        ...
        <details class="salon-details salon-details--tech">
          <summary>Ustawienia techniczne — tylko dla programisty</summary>
          <p class="salon-field-hint">Plik po wgraniu — zwykle sam się tu pojawia.</p>
          <div class="salon-field">
            <label>Adres pliku po wgraniu</label>
            <input type="text" data-field="src" value="${esc(src)}" />
          </div>
        ...
        <div class="salon-card-preview-block">
          ...
          <div class="salon-card-preview-card">
            ...
            <p data-card-preview-desc></p>
            <button type="button" class="salon-preview-fake-btn" data-card-preview-btn>Wybierz</button>
          </div>
        </div>
  ...
  function updateLandingPreview(wrap, item) {
    ...
    const mini = wrap.querySelector(".salon-card-preview-media");
    const hero = wrap.querySelector(".salon-card-hero__media");
    const mainPrev = wrap.querySelector(".salon-preview--main");
    const mediaHtml = () => {
      if (!src) return "";
      if (isVid) {
        return `<video class="salon-preview" src="${esc(src)}" muted loop playsinline controls></video>`;
      }
    ...
    const setBox = (box) => {
      ...
      if (src) {
        ...
        box.innerHTML = `<span class="salon-preview-placeholder">${esc(item.visualClass || "styl")}</span>`;
      }
    };
    setBox(mini);
    setBox(hero);
...
function buildBookingServiceCardHtml(svc, svcIdx, cfg) {
  ...
  const bChecks = (cfg.barbers || [])
    .map((b) => {
      ...
      return `<label class="salon-check"><input type="checkbox" data-svc-barber="${esc(
        b.id
  ...
  const promoDays = [1, 2, 3, 4, 5, 6, 0]
    .map((d) => {
      ...
      return `<label class="salon-check"><input type="checkbox" data-svc-promo-day="${d}" ${on} /><span>${discDayShort[d]}</span></label>`;
    })
    .join("");
  ...
  return `
      <details class="salon-subcard salon-subcard--booking-svc salon-collapsible-svc salon-search-mark" data-b-svc="${svcIdx}">
        <summary class="salon-collapsible-summary">
          <span class="salon-collapsible-title salon-booking-svc-title">${esc(servicePlName(svc) || "Usługa")} <span class="salon-marker-chip">${esc(marker)}</span></span>
          <span class="salon-collapsible-meta">${Number(svc.basePrice) || 0} zł · ${esc(svc.duration || "—")}</span>
        </summary>
        <div class="salon-collapsible-body">
        <div class="salon-card-hero__actions" style="margin-bottom:8px;">
          <button type="button" class="salon-btn salon-btn--danger" data-svc-del="${svcIdx}">Usuń usługę</button>
        </div>
        ${fieldLocaleBlock(svc, "name", "bookingSvcName")}
        ${fieldLocaleBlock(svc, "description", "bookingSvcDesc")}
        <div class="salon-field"><label>Cena normalna (zł)</label><input type="number" data-svc-price value="${Number(svc.basePrice) || 0}" min="0" step="1" /></div>
        <div class="salon-field"><label>Cena po zniżce na stałe (zł) — opcjonalnie</label><input type="number" data-svc-disc step="0.01" value="${svc.discountedPrice === "" || svc.discountedPrice == null ? "" : esc(String(svc.discountedPrice))}" /></div>
        <p class="salon-field-hint">Niższa cena obok głównej — jako „albo po promocji”, gdy wpiszesz kwotę.</p>
        <div class="salon-field"><label>Czas na karcie (np. 1 h)</label><input type="text" data-svc-dur value="${esc(svc.duration || "")}" placeholder="1 h" /></div>
        <div class="salon-field"><label>Czas trwania w minutach</label><div class="salon-inline-num"><input type="number" data-svc-min value="${Number(svc.durationMinutes) || 0}" min="1" /><span>min</span></div></div>
        <div class="salon-field"><label>Kategoria (w której pokazać usługę)</label><select data-svc-cat>${catOptions}</select></div>
        <div class="salon-field salon-field--inline"><label><input type="checkbox" data-svc-vis ${svc.visible !== false ? "checked" : ""} /> Pokazuj w rezerwacji</label></div>
        <div class="salon-field salon-field--inline"><label><input type="checkbox" data-svc-any ${svc.allowClientBarberChoice !== false ? "checked" : ""} /> Klient może wybrać barbera z listy</label></div>
        <p class="salon-field-hint">Kto może wykonać tę usługę?</p>
        <div class="salon-check-grid" data-svc-barbers>${bChecks}</div>

        <div class="salon-promo-service-box salon-subcard salon-subcard--inner">
          <h6 class="salon-mini-title">Zniżka czasowa tylko dla tej usługi</h6>
          <p class="salon-field-hint salon-field-hint--tight">Opcjonalnie — według dni i godzin.</p>
          <div class="salon-field salon-field--inline"><label><input type="checkbox" data-svc-promo-enabled ${p.enabled ? "checked" : ""} /> Włączyć zniżkę dla tej usługi</label></div>
          <div class="salon-field salon-field--inline">
          ...
          <div class="salon-field"><label>Ile procent taniej?</label><input type="number" data-svc-promo-pct value="${pctVal}" min="0" max="100" /></div>
          <div class="salon-field"><label>Cena po zniżce (zł)</label><input type="number" data-svc-promo-price step="0.01" value="${esc(priceAfterVal)}" /></div>
          <div class="salon-field">
            <span class="salon-faux-label">Dni zniżki</span>
            <div class="salon-disc-days">${promoDays}</div>
          </div>
          <div class="salon-field"><label>Godziny zniżki od</label><input type="time" data-svc-promo-start value="${esc(minutesToTime(typeof p.startMinutes === "number" ? p.startMinutes : 600))}" /></div>
          <div class="salon-field"><label>Godziny zniżki do</label><input type="time" data-svc-promo-end value="${esc(minutesToTime(typeof p.endMinutes === "number" ? p.endMinutes : 960))}" /></div>
        </div>

        <details class="salon-details salon-details--tech">
          <summary>Ustawienia techniczne — tylko dla programisty</summary>
          <p class="salon-field-hint">Wewnętrzny zapis w systemie.</p>
          <div class="salon-field"><label>Wewnętrzny identyfikator</label><input type="text" readonly value="${esc(svc.id)}" /></div>
        </details>
        </div>
      </details>`;
}
...
function renderBookingEditor(container) {
  ...
  const dayRows = [
    ...
    .map((d) => {
      ...
      return `
      <div class="salon-hours-row" data-day="${esc(d.k)}">
        <strong class="salon-hours-day">${esc(d.label)}</strong>
        <label class="salon-check"><input type="checkbox" data-oh-open ${salonOpen ? "checked" : ""} /> Salon otwarty</label>
        <div class="salon-hours-hours">
  ...
  const catBlocksNested = (cfg.serviceCategories || [])
    ...
    .map(({ cat, idx }) => {
      ...
      const svcInner = (cfg.services || [])
        ...
        .join("");
      return `
      <details class="salon-booking-cat-block salon-subcard salon-collapsible-cat salon-search-mark" data-b-cat="${idx}" data-booking-anchor>
        <summary class="salon-collapsible-summary salon-collapsible-summary--cat">
          <span class="salon-collapsible-title salon-booking-cat-heading">Kategoria: ${esc(pickLocAdmin(cat.title, "pl") || "—")} <span class="salon-marker-chip">${esc(getCategoryMarkerLabel(cat))}</span></span>
          <span class="salon-collapsible-meta">${(cfg.services || []).filter((s) => s.category === cat.id).length} usług</span>
        </summary>
        <div class="salon-collapsible-body">
        ...
        <div class="salon-field">
          ...
          <p class="salon-field-hint">Niższa liczba = wyżej na liście.</p>
        </div>
        <div class="salon-field">
          <label>Szybki kolor / emoji kategorii</label>
          <div class="salon-marker-row">
            ${markerSelectHtml(cat.marker, 'data-cat-marker')}
            <button type="button" class="salon-btn salon-btn--ghost salon-btn--marker-plus" data-cat-second-toggle title="Dodaj drugi kolor">+</button>
          </div>
          <div class="salon-marker-row salon-marker-row--second" data-cat-second-wrap ${cat.marker2 ? "" : 'style="display:none;"'}>
            ${markerSelectHtml(cat.marker2 || "🔵", 'data-cat-marker2')}
            <button type="button" class="salon-btn salon-btn--ghost salon-btn--marker-plus" data-cat-second-remove title="Usuń drugi kolor">×</button>
          </div>
          <p class="salon-field-hint">Ten sam marker (1 lub 2 kolory) automatycznie pojawi się przy wszystkich usługach tej kategorii.</p>
        </div>
        <details class="salon-details salon-details--tech"><summary>Ustawienia techniczne — tylko dla programisty</summary>
          <p class="salon-field-hint">Wewnętrzny zapis kategorii.</p>
          <div class="salon-field"><label>Wewnętrzny identyfikator</label><input type="text" readonly value="${esc(cat.id)}" /></div>
        </details>
        <div class="salon-field salon-field--tight">
          <button type="button" class="salon-btn salon-btn--danger salon-btn--block-sm" data-cat-del="${idx}">Usuń kategorię</button>
        </div>
        <div class="salon-booking-cat-services">
          <h5 class="salon-panel-subtitle salon-panel-subtitle--nest">Usługi w tej kategorii</h5>
          <div class="salon-stack salon-stack--nest">${svcInner || `<p class="salon-hint">Brak usług — dodaj nową poniżej lub przenieś tu istniejącą zmianą kategorii.</p>`}</div>
        </div>
        </div>
      </details>`;
    })
  ...
  const weekdayChecks = [1, 2, 3, 4, 5, 6, 0]
    .map((d) => {
      ...
      return `<label class="salon-check"><input type="checkbox" data-disc-day="${d}" ${on} /><span>${discDayShort[d]}</span></label>`;
    })
    .join("");

  container.innerHTML = `
    <div class="salon-item salon-subcard salon-booking-intro" id="booking-intro">
      <p class="salon-hint">Tu ustawiasz usługi w kreatorze rezerwacji, godziny pracy i zniżki. Zapis: „Zapisz zmiany” na górze.</p>
      <div class="salon-booking-quicknav">
        <button type="button" class="salon-btn salon-btn--ghost" data-booking-jump="booking-hours">Godziny pracy</button>
        <button type="button" class="salon-btn salon-btn--ghost" data-booking-jump="booking-discount">Zniżka</button>
        <button type="button" class="salon-btn salon-btn--ghost" data-booking-jump="booking-catalog">Kategorie i usługi</button>
      </div>
      <div class="salon-booking-actions">
        <button type="button" class="salon-btn salon-btn--ghost salon-btn--block-sm" id="bookingResetDefaults">Przywróć ustawienia jak na początku</button>
        <button type="button" class="salon-btn salon-btn--primary salon-btn--block-sm salon-btn--plus" id="bookingAddCategory">+ Dodaj kategorię usług</button>
        <button type="button" class="salon-btn salon-btn--primary salon-btn--block-sm salon-btn--plus" id="bookingAddService">+ Dodaj usługę do rezerwacji</button>
      </div>
    </div>

    <h3 class="salon-panel-subtitle" id="booking-hours" data-booking-anchor>Godziny pracy salonu</h3>
    <p class="salon-field-hint">Na podstawie tych godzin pokazujemy wolne terminy.</p>
    <div class="salon-item salon-hours">${dayRows}</div>

    <h3 class="salon-panel-subtitle" id="booking-discount" data-booking-anchor>Zniżka ogólna (dla wielu usług naraz)</h3>
    <div class="salon-item salon-subcard">
      <div class="salon-field salon-field--inline"><label><input type="checkbox" id="disc-enabled" ${disc.enabled !== false ? "checked" : ""} /> Włączyć ogólną zniżkę?</label></div>
      <div class="salon-field"><label>Nazwa dla Ciebie (nie musi być na stronie)</label><input type="text" id="disc-name" value="${esc(disc.name || "")}" /></div>
      <div class="salon-field"><label>Krótki tekst dla klienta</label><textarea id="disc-msg">${esc(disc.clientMessage || "")}</textarea></div>
      <div class="salon-field"><label>Ile procent zniżki?</label><input type="number" id="disc-pct" value="${Number(disc.percent) || 10}" min="0" max="100" /></div>
      <p class="salon-field-hint">Przykład: poniedziałek–czwartek, 10:00–16:00.</p>
      <div class="salon-field">
        <span class="salon-faux-label">W jakie dni działa zniżka?</span>
        <div class="salon-disc-days">${weekdayChecks}</div>
      </div>
      <div class="salon-field"><label>Od której godziny?</label><input type="time" id="disc-start-time" value="${esc(minutesToTime(disc.startMinutes != null ? disc.startMinutes : 600))}" /></div>
      <div class="salon-field"><label>Do której godziny?</label><input type="time" id="disc-end-time" value="${esc(minutesToTime(disc.endMinutes != null ? disc.endMinutes : 960))}" /></div>
    </div>

    <h3 class="salon-panel-subtitle" id="booking-catalog" data-booking-anchor>Kategorie i usługi w rezerwacji</h3>
    <p class="salon-field-hint salon-field-hint--tight">Kategoria to „folder” — w środku widać tylko usługi z niej.</p>
    <div class="salon-stack">${catBlocksNested}</div>

    <div class="salon-sheet-backdrop" id="bookingAddSvcSheet" aria-hidden="true">
      <div class="salon-sheet salon-sheet--booking" role="dialog" aria-modal="true" aria-labelledby="bookingAddSvcSheetTitle">
        <button type="button" class="salon-sheet-close" data-close-add-svc-sheet aria-label="Zamknij">×</button>
        <h4 id="bookingAddSvcSheetTitle">Do której kategorii?</h4>
        <p class="salon-field-hint">Wybierz kategorię, a od razu przejdziesz do edycji nowej usługi.</p>
        <div class="salon-sheet-list" data-add-svc-cats></div>
      </div>
    </div>
  `;
  ...
  function syncHoursFromForm() {
    ...
    container.querySelectorAll(".salon-hours-row").forEach((row) => {
  ...
  function bindBookingForm() {
    ...
    container.querySelectorAll("[data-b-cat]").forEach((el) => {
      ...
      el.querySelector("[data-cat-marker]")?.addEventListener("change", (e) => {
        ...
        const h = el.querySelector(".salon-booking-cat-heading");
    ...
    container.querySelectorAll("[data-b-svc]").forEach((el) => {
      ...
      bindPlCopyEmptyButtons(el, svc, () => {
        const h = el.querySelector(".salon-booking-svc-title");
    ...
    container.querySelectorAll(".salon-hours-row input").forEach((inp) => {
  ...
  container.querySelector("#bookingAddCategory")?.addEventListener("click", () => {
    ...
    requestAnimationFrame(() => {
      ...
      el?.classList.add("salon-card--flash");
      window.setTimeout(() => el?.classList.remove("salon-card--flash"), 1600);
  ...
  function addBookingServiceInCategory(categoryId) {
    ...
    requestAnimationFrame(() => {
      ...
      catEl?.classList.add("salon-card--flash");
      svcEl?.classList.add("salon-card--flash");
      window.setTimeout(() => {
        catEl?.classList.remove("salon-card--flash");
        svcEl?.classList.remove("salon-card--flash");
      }, 1600);
  ...
  function openAddServiceCategorySheet() {
    ...
    const panel = sheet?.querySelector(".salon-sheet--booking");
    ...
    if (!categories.length) {
      listEl.innerHTML = `<p class="salon-hint">Na razie nie masz utworzonych kategorii.</p>`;
    ...
    listEl.innerHTML = categories
      .map(
        (cat) => `<button type="button" class="salon-btn salon-btn--ghost salon-btn--block-sm" data-add-svc-cat-id="${esc(cat.id)}">
...
function renderContactsEditor(container) {
  ...
  ensureLandingHero();
  const lh = state.landingHero;
  ensureLocaleRef(lh, "alt");
  container.innerHTML = `
    <div class="salon-item salon-subcard" id="contacts-block-hero">
      <h3 class="salon-mini-title">Zdjęcie hero (start strony)</h3>
      <p class="salon-hint">Duże zdjęcie obok nagłówka — desktop i mobile.</p>
      <div class="salon-field">
        <label>Podgląd</label>
        <div id="hero-admin-preview" class="salon-preview-box salon-hero-admin-preview"></div>
      </div>
      <div class="salon-field">
        <label>Ścieżka pliku</label>
        <input type="text" id="hero-image-src" value="${esc(lh.image || "")}" placeholder="/assets/admin-uploads/..." />
      </div>
      <div class="salon-field">
        <label>Wgraj zdjęcie</label>
        <input type="file" id="hero-image-upload" accept="image/jpeg,image/png,image/webp,image/gif" />
      </div>
      <div class="salon-field salon-field--inline">
        <button type="button" class="salon-btn salon-btn--ghost" id="hero-image-clear">Wyczyść hero</button>
      </div>
      <h4 class="salon-section-title">Tekst alternatywny (dostępność)</h4>
      ${fieldLocaleBlock(lh, "alt", "heroAlt")}
    </div>
    <div class="salon-item salon-subcard" id="contacts-block-kontakt">
      <h3 class="salon-mini-title">Zmień telefon, adres i linki</h3>
      <p class="salon-hint">Te informacje widzą klienci na stronie i w stopce.</p>
      <div class="salon-field">
      ...
      <h4 class="salon-section-title">Adres salonu</h4>
      ...
      <div class="salon-field">
        ...
        <p class="salon-field-hint">Ten link otworzy opinie salonu w Google.</p>
      </div>
      <div class="salon-field">
      ...
      <details class="salon-details salon-details--tech">
        <summary>Dodatkowe ustawienia — nie musisz tego ruszać</summary>
        <p class="salon-field-hint">Numer do kliknięcia „zadzwoń” — bez spacji.</p>
        <div class="salon-field">
        ...
        <div class="salon-field">
          ...
          <p class="salon-field-hint">Jeśli ustawisz plik (np. <code>/assets/ui/map.jpg</code>), strona pokaże go zamiast iframe.</p>
        </div>
      </details>
    </div>
    <div class="salon-item salon-subcard">
      <h3 class="salon-mini-title">Social media</h3>
      <div class="salon-field">
  ...
  bindLocalePath(container.querySelector("#contacts-block-hero"), lh);
  bindLocalePath(container.querySelector("#contacts-block-kontakt"), c);

  const heroPrev = container.querySelector("#hero-admin-preview");
  const syncHeroPreview = () => {
    if (!heroPrev) return;
    const u = (lh.image || "").trim();
    heroPrev.style.backgroundImage = u ? `url("${u.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}")` : "";
  };
  syncHeroPreview();

  container.querySelector("#hero-image-src")?.addEventListener("input", (e) => {
    lh.image = e.target.value.trim();
    syncHeroPreview();
    updateDirtyBanner();
  });
  container.querySelector("#hero-image-upload")?.addEventListener("change", async (e) => {
    const file = e.target.files && e.target.files[0];
    if (!file) return;
    try {
      const path = await uploadFile(file);
      lh.image = path;
      const inp = container.querySelector("#hero-image-src");
      if (inp) inp.value = path;
      syncHeroPreview();
      showSave("Wgrano zdjęcie hero", "ok");
      updateDirtyBanner();
    } catch (err) {
      showSave(err.message || "Upload failed", "err");
    }
    e.target.value = "";
  });
  container.querySelector("#hero-image-clear")?.addEventListener("click", () => {
    lh.image = "";
    lh.alt = { pl: "", en: "", ru: "" };
    renderContactsEditor(container);
    updateDirtyBanner();
  });
...
function setActiveTab(id) {
  document.querySelectorAll(".salon-tab").forEach((b) => b.classList.toggle("active", b.dataset.tab === id));
  document.querySelectorAll(".salon-panel").forEach((p) => p.classList.toggle("active", p.dataset.panel === id));
...
function buildTabs() {
  ...
  nav.innerHTML = TABS.map(
    (t) => `<button type="button" class="salon-tab${t.id === "works" ? " active" : ""}" data-tab="${t.id}">${t.label}</button>`
  ).join("");
  nav.querySelectorAll(".salon-tab").forEach((btn) => {
    btn.addEventListener("click", () => setActiveTab(btn.dataset.tab));
  });
}

async function apiLoad() {
  const res = await fetch("/api/admin/content", fetchCred({ headers: authHeaders() }));
  if (res.status === 401) {
    setToken("");
...
async function apiSave() {
  const res = await fetch("/api/admin/content", fetchCred({
    method: "POST",
...
async function tryBoot() {
  try {
    const s = await fetch("/api/admin/session", fetchCred());
    if (!s.ok) {
...
document.getElementById("logoutBtn")?.addEventListener("click", async () => {
  try {
    await fetch("/api/admin/logout", fetchCred({ method: "POST" }));
  } catch (_e) {
    /* ignore */
  }
  setToken("");

...
function tBook(key, vars) {
  ...
  const root = window.SALON_TRANSLATIONS;
  if (!root) return key;
...
function getWorkingHoursForDate(dateStr) {
  const day = getWeekday(dateStr);
  const oh = typeof window !== "undefined" ? window.SALON_BOOKING_OPENING_HOURS : null;
...
function isDiscountWindow(dateStr, timeStr) {
  ...
  const cfg = typeof window !== "undefined" ? window.SALON_BOOKING_DISCOUNT : null;
...
function nextStep() {
  if (state.step === 1) {
    ...
    const pre =
      typeof window !== "undefined" && window.__SALON_BOOKING_PRESELECT_SERVICE
        ? String(window.__SALON_BOOKING_PRESELECT_SERVICE).trim()
        : "";
    if (pre) {
...
function salonSyncBookingStep1Nav() {
  if (state.step !== 1) return;
  syncContactFromInputs();
  updateBindings();
  updateNav();
...
window.salonSyncBookingStep1Nav = salonSyncBookingStep1Nav;

function refreshBookingWizardLanguage() {
  if (!stepTitle) return;
  if (state.step === 1) salonSyncBookingStep1Nav();
...
window.SALON_applyBookingConfigFromContent = function (contentParam) {
  const root =
    contentParam && typeof contentParam === "object"
      ? contentParam
      : typeof window !== "undefined"
        ? window.SALON_SITE_CONTENT
        : null;
  ...
  if (!raw || typeof raw !== "object") {
    window.SALON_BOOKING_OPENING_HOURS = null;
    window.SALON_BOOKING_DISCOUNT = null;
  ...
  if (raw.discount && typeof raw.discount === "object") {
    window.SALON_BOOKING_DISCOUNT = cloneBookingValue(raw.discount);
  ...
  if (raw.openingHours && typeof raw.openingHours === "object") {
    window.SALON_BOOKING_OPENING_HOURS = cloneBookingValue(raw.openingHours);
  } else {
    window.SALON_BOOKING_OPENING_HOURS = null;
  }
...
function mergeBookingCatalogSnapshot() {
  try {
    const admin = window.SALON_ADMIN_DATA;
